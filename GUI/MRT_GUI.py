...
mrtstate.offsets["eloff"] = 0.0
mrtstate.offsets["azoff"] = -180.0
...
def cmdCurrentRefresh(labelAzimuth, labelElevation, labelPower):
    ...
    labelElevation.config(
        text="Elevation:  " + str(mrtstate.state["elDeg"][0]) + " deg"
    )
...
def cmdQuickMove(direction, increment):
    if direction == "UP":
        mrtf.GoTo(
            ser,
            float(mrtstate.state["azDeg"][0]),
            float(mrtstate.state["elDeg"][0]) + increment,
        )
    elif direction == "DOWN":
        mrtf.GoTo(
            ser,
            float(mrtstate.state["azDeg"]),
            float(mrtstate.state["elDeg"]) - increment,
        )
    elif direction == "CCW":
        mrtf.GoTo(
            ser,
            float(mrtstate.state["azDeg"]) + increment,
            float(mrtstate.state["elDeg"]),
        )
    elif direction == "CW":
        mrtf.GoTo(
            ser,
...
class tabConnection(ttk.Frame):
    def __init__(self, parent):
        ...
        optionMenuSerialPort = ttk.OptionMenu(
            frameConnection, varSerialPort, "Select port...", *optionListSerialPort
        )
        ...
        optionBaudrate = ttk.OptionMenu(
            frameConnection, varBaudrate, optionListBaudrate[7], *optionListBaudrate
        )
        optionBaudrate.grid(row=4, column=0, columnspan=2, sticky="nsew")
        buttonConnect = ttk.Button(
            frameConnection, text="Connect", command=lambda: mrtf.connectToArduino()
        )
...
class tabState(ttk.Frame):
    def __init__(self, parent):
        ...
        labelEstimatedTotalTime = ttk.Label(
            frameState, text="Estimated Total Time: 0:00"
        )
        ...
        labelEstimatedTotalTime.grid(
            row=2, column=0, padx=10, pady=10, sticky="we", columnspan=3
        )
        labelElapsedTime.grid(
            row=4, column=0, padx=10, pady=10, sticky="we", columnspan=3
        )
...
class tabControl(ttk.Frame):
    def __init__(self, parent):
        ...
        labelframeDegreesIncrement = ttk.Labelframe(
            labelframeQuickMove, text="Increment"
        )
        ...
        """ Widgets """
        # Label
        labelCurrentAzimuth = ttk.Label(
            labelframeCurrentReading, text="Azimuth:  0.0 deg"
        )
        labelCurrentElevation = ttk.Label(
            labelframeCurrentReading, text="Elevation:  0.0 deg"
        )
        ...
        buttonRefresh = ttk.Button(
            labelframeCurrentReading,
            text="Refresh",
            command=lambda: cmdCurrentRefresh(
                labelCurrentAzimuth, labelCurrentElevation, labelCurrentPower
            ),
        )

        # Radiobutton
        radiobutton1 = ttk.Radiobutton(
            labelframeDegreesIncrement,
            text="1 deg",
            variable=varDegreeIncrement,
            value=1,
        )
        ...
        radiobutton10 = ttk.Radiobutton(
            labelframeDegreesIncrement,
            text="10 deg",
            variable=varDegreeIncrement,
            value=10,
        )
        radiobutton20 = ttk.Radiobutton(
            labelframeDegreesIncrement,
            text="20 deg",
            variable=varDegreeIncrement,
            value=20,
...
class tabScan(ttk.Frame):
    def __init__(self, parent):
        ...
        # Entry
        entryScanStartAzimuth = ttk.Entry(
            frameScanControl, textvariable=varScanStartAzimuth
        )
        entryScanStartElevation = ttk.Entry(
            frameScanControl, textvariable=varScanStartElevation
        )
        ...
        buttonScan = ttk.Button(
            frameScanControl,
            text="Scan",
            command=lambda: cmdScan(
                varScanStartAzimuth.get(),
                varScanStartElevation.get(),
                varScanDirection.get(),
                varScanAmount.get(),
                labelScanMessage,
            ),
        )
        ...
        canvasScan.draw()
        canvasScan.get_tk_widget().pack(
            side=tk.BOTTOM, fill=tk.BOTH, expand=True, anchor=tk.S
        )
        ...
        canvasScan.get_tk_widget().pack(
            side=tk.TOP, fill=tk.BOTH, expand=True, anchor=tk.S
        )
...
class tabMap(ttk.Frame):
    def __init__(self, parent):
        ...
        """ Entry """
        entryMapStartAzimuth = ttk.Entry(
            frameMapControl, textvariable=varMapCenterAzimuth
        )
        entryMapStartElevation = ttk.Entry(
            frameMapControl, textvariable=varMapCenterElevation
        )
        ...
        buttonMap = ttk.Button(
            frameMapControl,
            text="Map",
            command=lambda: cmdMap(
                entryMapStartAzimuth.get(),
                entryMapStartElevation.get(),
                entryMapHeight.get(),
                entryMapWidth.get(),
                labelMapMessage,
            ),
