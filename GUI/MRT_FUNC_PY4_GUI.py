...
def numpyState(state):
    ...
    for i in np.arange(len(mrtstate.state_vars)):
        ndata[mrtstate.state_vars[i]] = np.array(
            state[mrtstate.state_vars[i]], dtype=mrtstate.state_dtypes[i]
        )
    ndata["pwr"] = mrt.zx47_60(ndata["voltage"])
    # Both readState and readStream run through here.
    # Apply offsets
    ndata["azDeg"] = np.round(
        np.mod(-ndata["azDeg"] - mrtstate.offsets["azoff"], 360), 3
    )
...
def GoTo(ser, azG=None, elG=None):
    ...
    if float(azG) >= 0.0 and float(azG) <= 360.0:
        az_ok = True
    else:
        ...
        if var == "Y":
            ...
            az_ok = False
    if float(elG) >= -mrtstate.offsets["eloff"] and float(elG) <= 120.0:
        el_ok = True
    else:
...
def GoEl(ser, elGe=None):
    ...
    print("d_el: ", d_elge)
    if elGe >= -mrtstate.offsets["eloff"] and elGe <= 120.0:
        el_ok = True
    else:
        print("Requested elevation out of bounds")
        var = input("Are you sure? (Y/N) ")
...
def RasterMap(ser, az, el, azd, eld):
    ...
    np.savez(
        file="map_" + time.ctime().replace(" ", "_") + ".npz",
        az=az,
        el=el,
        pwr=pwr,
        zi=zi,
