...
def cmdScan():
    ...
    # Save
    np.savez(file=time.ctime().replace(" ", "_") + ".npz", ndata=ndata)
    # Plot
    PlotData(ndata)
...
def numpyState(state):
    ...
    for i in np.arange(len(state_vars)):
        ndata[state_vars[i]] = np.array(state[state_vars[i]], dtype=state_dtypes[i])
...
def GoTo(azG=None, elG=None):
    ...
    if azG >= 0.0 and azG <= 360.0:
        ...
        if var == "Y":
            ...
            az_ok = False
    if elG >= -offsets["eloff"] and elG <= 120.0:
        el_ok = True
    else:
...
def GoEl(elGe=None):
    ...
    print("d_el: ", d_elge)
    if elGe >= -offsets["eloff"] and elGe <= 120.0:
        el_ok = True
    else:
        print("Requested elevation out of bounds")
        var = input("Are you sure? (Y/N) ")
